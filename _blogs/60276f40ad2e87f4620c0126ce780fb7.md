...
## Principle
The method in this blog is the same as the principle of SD card copier: divide the target hard disk into two areas, the boot area (named `bootfs`, format Windows_FAT_32) and the usage area (also called the root area named `rootfs` , the format is Linux), and then use `rsync` to copy, which is much faster.
