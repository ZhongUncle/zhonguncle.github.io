...
excerpt: "This blog explains how to use C++ to verify the central limit theorem (CLT), which states that the sum of independent random variables tends to follow a normal distribution when the sample size is large enough. ZhongUncle uses dice points as an example of discrete events, and generates PPM format images to visualize the probability distribution of the sum of points. ZhongUncle shows that the distribution curve becomes more similar to a normal curve as the number of samples increases, and that this holds true even when the probability of each dice point is not uniform. ZhongUncle provides the source code and the output images for different scenarios, and invites the readers to try it themselves. ZhongUncle also expresses the fascination with the central limit theorem and the normal distribution."
---
